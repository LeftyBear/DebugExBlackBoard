...
Public Function NormalizeToDate(ByVal RawText As String) As Date
    ...
    If 0 < VBA.InStr(1, TextValue, charBackSlash) Then
        If UBound(VBA.Split(TextValue, charBackSlash)) <> 2 Then Err.Raise Util_ErrNum.InvalidDateFormat, "Util_Function", "日付形式不正: " & RawText
        Dim YearPart As Long
        YearPart = CLng(VBA.Split(TextValue, charBackSlash)(0))
        Dim MonthPart As Long
        MonthPart = CLng(VBA.Split(TextValue, charBackSlash)(1))
        Dim DayPart As Long
        DayPart = CLng(VBA.Split(TextValue, charBackSlash)(2))
